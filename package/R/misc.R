...
f.save_tsv <- function(dat, file_out, config, row.names=T, col.names=T) {
  tryCatch(
    ...
    error=function(msg) f.err("write.table() error: writing to ", file_out, ": ", msg$message, config=config),
    warning=function(msg) f.err("write.table() warning: writing to ", file_out, ": ", msg$message, config=config)
  )
}

f.check_state <- function(state, config) {
  if(!all(rownames(exprs) == feats[, config$feat_id_col, drop=T])) {
    f.err("!all(rownames(exprs) == feats[, config$feat_id_col])", config=config)
  }
  if(!all(colnames(exprs) == samps[, config$obs_col, drop=T])) {
    f.err("!all(colnames(exprs) == samps[, config$obs_col])", config=config)
  }
}
...
f.save_state <- function(state, config, prefix) {
  ...
  f.log("writing exprs data to", file_out, config=config)
